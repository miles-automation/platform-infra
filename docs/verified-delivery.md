...
Deploy-event project lookup uses `/api/v1/task-sparks/{identifier}` to resolve minimal identity metadata under existing task permissions (Spark task612). It does not request full spark configuration or fall back to a master key. Lookup or event authorization failures remain visible warnings; the delivery driver must still retain its verified task/build/release receipts. Deploy task612 before verifying project-linked events with the updated CLI.
